...
df_businessId = df_review.where("user_id = \"SIfJLNMv7vBwo-fSipxNgg\"").select("business_id")
...
df_gps.rdd.map(lambda r: ",".join([str(c) for c in r])).repartition(1).saveAsTextFile("/home/sanchit/project/yelp/data/yelp_dataset_challenge_academic_dataset/reviewDataSet/spark-scripts/Yelp-Challenge/gps_coordinats1.txt")
